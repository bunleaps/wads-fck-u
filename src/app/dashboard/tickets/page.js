  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      ...
      if (user.role === 'admin') {
        ...
        apiUrl = `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user/tickets`; // Example user endpoint
      } else {
      ...
      try {
        ...
        setError(err.response?.data?.message || `Failed to load tickets for ${user.role}.`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">
        Tickets Management {user && `(${user.role === 'admin' ? 'Admin View' : 'My Tickets'})`}
      </h1>
      {user && (
      ...
      {user && user.role === 'admin' && (
        <div className="mb-4 p-2 bg-blue-100 border border-blue-300 rounded">Admin-specific controls or information can go here.</div>
      )}
      {user && user.role === 'user' && (
        <div className="mb-4 p-2 bg-green-100 border border-green-300 rounded">User-specific actions like "Create New Ticket" can go here.</div>
      )}
...
// Protect this page: only allow users with the 'admin' role.
